...
**Lag feature-ების გაჟონვა (leakage) inference-ის დროს**: პირველადმა Pipeline-ის იმპლემენტაციამ Kaggle-ის test set-ზე გაცილებით ცუდი შედეგი აჩვენა (Public WMAE ~18,363), ვიდრე ვალიდაციაზე (1328.40). პირველი მიზეზი იყო ის, რომ `Sales_Lag_1`/`Sales_Lag_52` ყოველთვის იყო 0 test set-ზე — გამოსწორდა `history_df`-ის გადაცემით `FeatureBuilder`-ში (Public WMAE გაუმჯობესდა ~18,363-დან ~12,638-მდე).

თუმცა დარჩა უფრო ღრმა პრობლემა: Kaggle-ის test set მოიცავს 39-ზე მეტ კვირას მომავალში (multi-step forecasting), ხოლო `Sales_Lag_1` ("წინა კვირის გაყიდვები") სწორად გამოითვლება მხოლოდ ყოველი Store/Dept-ის **პირველი** test კვირისთვის — შემდგომი კვირებისთვის საჭირო იქნებოდა **რეკურსიული პროგნოზირება** (predicted მნიშვნელობის გამოყენება შემდეგი კვირის lag-ად), რაც არ არის იმპლემენტირებული ამ ვერსიაში დროის შეზღუდვის გამო. ეს ხსნის, თუ რატომ რჩება Kaggle-ის public score მაინც მაღალი ვალიდაციასთან შედარებით — ვალიდაცია ხდება იმავე დისტრიბუციის მონაცემებზე სადაც lag-ები რეალურია, ხოლო test set-ზე ისინი მხოლოდ ნაწილობრივ არის ხელმისაწვდომი.

## Kaggle შედეგი

- პირველადი submission (bug-იანი lag feature-ებით): Public WMAE 18,363.56
- გასწორებული submission (history-based lag lookup): Public WMAE 12,638.01
- ცნობილი შეზღუდვა: რეკურსიული პროგნოზირება არ არის იმპლემენტირებული ამ ვერსიაში — შესაძლო გაუმჯობესების მიმართულება მომავლისთვის.
